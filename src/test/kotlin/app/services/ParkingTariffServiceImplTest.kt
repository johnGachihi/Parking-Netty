...
internal class ParkingTariffServiceImplTest {
    ...
    inner class TestGetOverlappingTariff {
        ...
        inner class TestWhenThereIsTariffData {
            ...
            @Test
            fun `and duration is larger than the upperLimit for all tariffs, then returns tariff with highest upperLimit`() {
        ...
        @Nested
        @DisplayName("When there is no parking tariff data")
        inner class WhenThereIsNoParkingTariffData {
            @BeforeEach
            fun initTariffs() {
                every {
                    parkingTariffRepo.getAllInAscendingOrder()
            ...
            @Test
            fun `then returns null`() {
                assertNull(
                    parkingTariffService.getOverlappingTariff(Minutes(1))
                )
            }
        }
    }
